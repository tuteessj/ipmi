...
void draw() {
  ...
  energia = constrain(energia, 0, 1);  // evita que se rompa
...
void dibujarCuadranteInteractivo(float x, float y, float w, float h,
  ...
  // cuando pongo en reposo la obra
  if (energia <= 0.01) {

    noStroke();

    for (int i = 0; i < cant; i++) {
      float tam = map(i, 0, cant - 1, w, 10);
  ...
  for (int i = 0; i < cant; i++) {
    ...
    float wave = sin(frameCount * 0.02 + i) * 6 * energia;
